...
import tests.http.HttpClient;
import tests.http.ServerURLBuilder;
...
public class CloudAppBridge implements AppBridge {
  ...
  public void updateEnvironmentResultData(TestDeviceResultRequest testDeviceResultRequest) throws AutomatorException {
    //TODO update environment result data
    // try {
  ...
  public TestCaseEntity getTestCase(Long environmentResultId, TestCaseEntity testCaseEntity) throws AutomatorException {
    try {
      ...
      Object a = AutomatorTest.readObjectAb(testCaseEntity.getTestCaseResultId().toString(), new TestCaseEntity());
      TestCaseEntity object = (TestCaseEntity)a;
      return object;
  ...
  public WebDriverSettingsDTO getWebDriverSettings(Long environmentResultId) throws AutomatorException {
    try {
      ...
      //HttpResponse<WebDriverSettingsDTO> response = webAppHttpClient.get(url, new TypeReference<>() {      }, authHeader);
      //if (response.getStatusCode() != HttpStatus.OK.value()) {
        //throw new AutomatorException(response.getStatusMessage());
      //}
      //return response.getResponseEntity();
      Object a = AutomatorTest.readObjectAb(environmentResultId.toString(), new WebDriverSettingsDTO());
      WebDriverSettingsDTO object = (WebDriverSettingsDTO)a;
      //return  AutomatorTest.readWebDriver("webDriverDTO");
      return object;
    } catch (Exception e) {
      log.error(e.getMessage(), e);
      //throw new AutomatorException(e.getMessage(), e);
